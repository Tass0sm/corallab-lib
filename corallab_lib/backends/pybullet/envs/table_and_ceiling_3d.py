...
class EnvTableAndCeiling3D:
    ...
    def setup(cls, env):
        ...
        env.table_dimensions = np.array([0.3, 1.2, 0.2])
        ...
        # ADD TABLE
        table_urdf_path = str(corallab_assets.get_resource_path("objects/table.urdf"))
        ...
        # ADD CEILING
        ceiling_height = 0.75
        ceiling_center = env.table_center + np.array([0.0, 0.0, ceiling_height])

        ceiling_urdf_path = str(corallab_assets.get_resource_path("objects/table.urdf"))
        env.ceiling_id = env.client.loadURDF(ceiling_urdf_path, basePosition=ceiling_center, useFixedBase=True)
        env.pb_objs.append(env.ceiling_id)
