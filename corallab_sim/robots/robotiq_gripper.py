...
class RobotiqGripper(Gripper):
    ...
    def __create_mimic_joints__(self):
        ...
        self.joint_to_mimic_id = [
            j[0] for j in self.joints if j[1] == joint_to_mimic_name
        ][0]

        for j_name, mult in mimic_joints_and_multipliers.items():
            self.__create_mimic_constraint__(joint_to_mimic_name, j_name, mult)

    def __create_mimic_constraint__(self, joint_to_mimic_name, joint_name, multiplier):
    ...
    def get_contact_points(self):
        ...
        left_pad_link_id = [
            j[0] for j in self.joints if j[1] == b"robotiq_2f_85_left_pad_joint"
        ][0]
        right_pad_link_id = [
            j[0] for j in self.joints if j[1] == b"robotiq_2f_85_right_pad_joint"
        ][0]

        left_contact_points = p.getContactPoints(
            bodyA=self.id, linkIndexA=left_pad_link_id
        )
        right_contact_points = p.getContactPoints(
            bodyA=self.id, linkIndexA=right_pad_link_id
        )
        points = [p for p in left_contact_points if p[2] != self.id] + [
            p for p in right_contact_points if p[2] != self.id
        ]
    ...
    def grasp(self, on, colmask, grasp_force_limit=15):
        # collision masks for simplifying testing
        # obj = self.check_grasp()

        def bc():
            return self.grasp_force() > grasp_force_limit
...
class RealRobotiqGripper:
    ...
    def is_active(self):
        ...
        return (
            RealRobotiqGripper.GripperStatus(status)
            == RealRobotiqGripper.GripperStatus.ACTIVE
        )
    ...
    def auto_calibrate(self, log: bool = True) -> None:
        ...
        if (
            RealRobotiqGripper.ObjectStatus(status)
            != RealRobotiqGripper.ObjectStatus.AT_DEST
        ):
    ...
    def move_and_wait_for_pos(
        ...
        while (
            RealRobotiqGripper.ObjectStatus(cur_obj)
            == RealRobotiqGripper.ObjectStatus.MOVING
        ):
            cur_obj = self._get_var(self.OBJ)
        ...
        final_obj = cur_obj
        return final_pos, RealRobotiqGripper.ObjectStatus(final_obj)
